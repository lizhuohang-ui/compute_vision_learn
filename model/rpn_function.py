...
from model.imgae_list import ImageList
...
class AnchorsGenerator(nn.Module):
    ...
    def grid_anchors(self, grid_size, strides):
        ...
        """
        anchors = []
        cell_anchoers = self.cell_anchors
        assert cell_anchoers is not None

        # 遍历每个预测特征层的grid_size，strides和cell_anchors
        for size, strides, base_anchors in zip(grid_size, strides, cell_anchoers):
            grid_height, grid_width = size
            stride_height, stride_width = size
    ...
    def cached_grid_anchors(self, grid_size, strides):
        # type: (List[List[int]], List[List[Tensor]]) -> List[Tensor]
        """将计算得到的所有anchors信息进行缓存"""
        key = str(grid_size) + str(strides)
        if key in self._cache:
            return self._cache[key]
        anchors = self.grid_anchors(grid_size, strides)
    ...
    def forward(self, image_list, feature_maps):
        ...
        # 获取每个预测特征层的尺寸(heigth, width)
        grid_sizes = list([feature_map.shape[-2:] for feature_map in feature_maps])

        # 获取输入图像的height和width, 这里的输入图像时resize之后的
        ...
        anchoes_over_all_feature_maps = self.cached_grid_anchors(grid_sizes, strides)
        ...
        for i, (image_height, image_width) in enumerate(image_list.image_sizes):
            ...
            for anchors_per_feature_map in anchoes_over_all_feature_maps:
...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    box_cls = torch.cat(box_cls_flattened, dim=1).flatten(0, -2) # start_dim, end_dim: 展平的开始维度和结束维度
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
                 ...
                 _pre_nms_top_n, _post_nms_top_n, nms_thresh, score_thresh=0.0):
        ...
        # use during testing
        self._pre_nms_top_n = _pre_nms_top_n
        self._post_nms_top_n = _post_nms_top_n
    ...
    def _get_top_n_idx(self, objectness, num_anchor_per_level):
        ...
        offset = 0
        # 遍历每个预测特征层上的预测目标概率信息
        for ob in objectness.split(num_anchor_per_level, 1):
    ...
    def filter_proposals(self, proposals, objectness, image_shape, num_anchors_per_level):
        ...
        final_scores = []
        # 遍历每张图像的相关预测信息
        for boxes, scores, lvl, img_shape in zip(proposals, objectness, levels, image_shape):
            # 调整预测的boxes信息，将越界的坐标调整到图片边界上
            boxes = box_ops.clip_boxes_to_image(boxes, img_shape)

            # 返回boxes满足宽，高都大于min_size的索引
            keep = box_ops.remove_small_boxes(boxes, self.min_szie)
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            if gt_boxes.numel() == 0:
                ...
                gt_boxes = gt_boxes.to(anchors_per_image.device)
                # 计算anchors与真实bbox的iou信息
                # set to self.box_similarity when https://github.com/pytorch/pytorch/issues/27495 lands
                match_quality_matrix = box_ops.box_iou(gt_boxes, anchors_per_image)
                # 计算每个anchors与gt匹配iou最大的索引（如果iou<0.3索引置为-1，0.3<iou<0.7索引为-2）
                matched_indexs = self.proposal_matcher(match_quality_matrix)
                # get the targets corresponding GT for each proposal
                # NB: need to clamp the indices because we can have a single
                # GT in the image, and matched_idxs can be -2, which goes
                # out of bounds
                matched_gt_boxes_per_image = gt_boxes[matched_indexs.clamp(min=0)]

                labels_per_image = matched_indexs >= 0
                labels_per_image = labels_per_image.to(torch.float32)

                # background (negative examples)
                bg_indices = matched_indexs == self.proposal_matcher.BELOW_LOW_THRESHOLD
                labels_per_image[bg_indices] = 0.0

                # discard indices that are between thresholds
                inds_to_discard = matched_indexs == self.proposal_matcher.BETWEEN_THRESHOLDS  # -2
    ...
    def compute_loss(self, objectness, pred_bbox_deltas, labels, regression_targets):
        ...
        sampled_pos_indexs, sampled_neg_indexs = self.fg_bg_sampler(labels)
        # 将一个batch中的所有正负样本List(Tensor)分别拼接在一起，并获取非零位置的索引
        sampled_pos_indexs = torch.where(torch.cat(sampled_pos_indexs, dim=0))[0]
        sampled_neg_indexs = torch.where(torch.cat(sampled_neg_indexs, dim=0))[0]

        # 将所有正负样本索引拼接在一起
        sampled_index = torch.cat([sampled_pos_indexs, sampled_neg_indexs], dim=0)
        ...
        # 计算边界框回归损失
        # box_loss = det_utils.smooth_li_loss(
        #     pred_bbox_deltas[sampled_pos_indexs],
        #     regression_targets[sampled_pos_indexs],
        #     beta=1 / 9,
        #     size_average=False,
        # ) / (sampled_index.numel())
        box_loss = (
        ...
        objectness_loss = F.binary_cross_entropy_with_logits(
            objectness[sampled_index], labels[sampled_index]
        )
