...
def _resize_image_onnx(image, self_min_size, self_max_size):
    ...
    image_shape = operators.shape_as_tensor(image)[-2:]
    min_size = torch.min(image_shape).to(dtype=torch.float32)
    max_size = torch.max(image_shape).to(dtype=torch.float32)
...
def _resize_image(image, self_min_size, self_max_size):
    ...
    min_size = float(torch.min(image_shape))
    max_size = float(torch.max(image_shape))
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        max_size = self.max_by_axis([list(image.shape) for image in images])
...
def resize_boxes(boxes, original_size, new_size):
    ...
    # Returns a tuple of all slices along a given dimension, already withount it.
