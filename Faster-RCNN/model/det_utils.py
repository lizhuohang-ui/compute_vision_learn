...
class BalancedPositiveNegativeSampler(object):
    ...
    def __call__(self, matched_indexs):
        ...
        neg_indexs = []
        # 遍历每张图像的matched_idxs
        for matched_indexs_per_image in matched_indexs:
            # >= 1的为正样本, nonzero返回非零元素索引
            positive = torch.where(torch.ge(matched_indexs_per_image, 1))[0]

            # = 0的为负样本
            negative = torch.where(torch.eq(matched_indexs_per_image, 0))[0]
            ...
            pos_idx_per_image_mask = torch.zeros_like(
                matched_indexs_per_image, dtype=torch.uint8
            )
            ...
            pos_indexs.append(pos_idx_per_image_mask)
            neg_indexs.append(neg_idx_per_image_mask)

        return pos_indexs, neg_indexs
...
class BoxCoder(object):
    ...
    def encode_single(self, reference_boxes, proposals):
        ...
        target = encode_boxes(reference_boxes, proposals, weights)
    ...
    def decode(self, rel_codes, boxes):
        ...
        perd_boxes = self.decode_single(rel_codes, concat_boxes)

        # 防止pred_boxes为空时导致reshape报错
        if box_sum > 0:
            perd_boxes = perd_boxes.reshape(box_sum, -1, 4)
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        # ymin
        pred_boxes2 = pred_ctr_y - torch.tensor(0.5, dtype=pred_ctr_x.dtype, device=pred_ctr_x.device) * pred_h
        # xmax
        pred_boxes3 = pred_ctr_x + torch.tensor(0.5, dtype=pred_ctr_x.dtype, device=pred_ctr_x.device) * pred_w
        # ymax
        pred_boxes4 = pred_ctr_y + torch.tensor(0.5, dtype=pred_ctr_x.dtype, device=pred_ctr_x.device) * pred_h
...
class Matcher(object):
    ...
    def set_low_quality_matches_(self, matches, all_matches, match_quality_matrix):
        ...
        per_index_to_updata = gt_pred_pairs_of_highest_quality[1]
        matches[per_index_to_updata] = all_matches[per_index_to_updata]


def smooth_li_loss(input, target, beta: float = 1./ 9, size_average: bool = True):
    """
