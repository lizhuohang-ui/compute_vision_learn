...
from backbone.fpn import FeaturePyramidNetwork, LastLevelMaxPool

class Bottleneck(nn.Module):
    expansion = 4

    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layers=None):
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        layers.append(block(self.in_channel, channel, downsample=downsample,
                            stride=stride, norm_layers=norm_layer))
        self.in_channel = channel * block.expansion

        for _ in range(1, block_num):
            layers.append(block(self.in_channel, channel, norm_layers=norm_layer))
...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    def __init__(self, model, return_layers):
        if not set(return_layers).issubset([name for name, _ in model.named_children()]):
            raise ValueError("return_layers ara not present in model")
